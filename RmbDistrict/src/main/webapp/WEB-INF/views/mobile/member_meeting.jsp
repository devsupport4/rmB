...
<html lang="en">
	<head>
		...
		<title> Member Meeting | Rotary Means Business Fellowship Bangalore  </title>				
	...
	<body class="homepage" ng-app="rcbs" ng-controller="memberMeetingCtrl" ng-init="getMembersMeetingsDetailById('<%= session.getAttribute("loginid") %>','<%= session.getAttribute("fromdate") %>','<%= session.getAttribute("todate") %>')" ng-cloak>
		<div class="mdl-layout mdl-js-layout mdl-layout--fixed-header">
			...
			<main class="mdl-layout__content">
				<div class="mdl-card__title mdl-color--darkblue mdl-color-text--white relative mdl-subheader">
					...
					<h2 class="mdl-card__title-text uppercase-font font-18"> Member's Meetings</h2>
					<a href="<%=request.getContextPath() %>/one_to_one" class="addicn"><i class="fa fa-plus-circle pull-right icnsize icnright" aria-hidden="true"></i></a>
				</div>
						...
						<section class="profile">
							<div class="row row-margin">												
								...
								<div class="col-xs-2" style="padding:5px;">
									<button type="submit" class="btn btn-primary"  style="margin-top: 0px; background-color: #005daa;" ng-click="getMeetingsByDate(<%= session.getAttribute("loginid") %>)"> <i class="fa fa-search"></i></button>													
								</div>																																	
							</div>
							<div class="page-content">							
											...
											<div class="panel-heading">
												<div class="row row-margin">
													<div class="col-xs-4 pl-0">
														...
														<h4 class="panel-title linelnt" style="font-size:13px"></h4>
													</div>
													<div class="col-xs-8 pl-0">
														<h4 class="panel-title linelnt" style="font-size:13px">Met with</h4>
														<h4 class="panel-title linelnt" style="font-size:13px">Initiated by</h4>
													</div>
												</div>
												
											</div>											
										</div>
										<div ng-repeat="item in getMemberMeetings">
											<div class="panel panel-default" >
												<div class="panel-heading" data-toggle="collapse" data-parent=".d-accordion" href="#whoweare{{item.oneToOneId}}">
													
													<div class="row row-margin">
														<div class="col-xs-4 pl-0">
															<h4 class="panel-title linelnt" style="font-size:14px">{{item.meetingDate}}</h4>
															<h4 class="panel-title linelnt" style="font-size:14px"></h4>
														</div>
														<div class="col-xs-7 pl-0">
															<h4 class="panel-title linelnt" style="font-size:13px">{{item.memberFirstName}} {{item.memberLastName}}</h4>
															<h4 class="panel-title linelnt" style="font-size:13px">{{item.inviteeFirstName}} {{item.inviteeLastName}}</h4>
														</div>
												...
												<div id="whoweare{{item.oneToOneId}}" class="panel-collapse collapse">
													<ul class="list-group">												
														
														<li class="list-group-item"><i class="fa fa-map-marker" aria-hidden="true"></i>	{{item.location}}</li>
														
														<li class="list-group-item"><i class="fa fa-commenting-o" aria-hidden="true"></i>{{item.conversationTopic}} </li>
													</ul>
												</div>
											</div>											
										</div>
