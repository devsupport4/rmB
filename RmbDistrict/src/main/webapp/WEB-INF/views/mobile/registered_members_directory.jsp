...
<html lang="en">
	...
	<body class="homepage" ng-app="rcbs" ng-controller="registrMembersDirectoryCtrl" ng-init="checkMemberregistration(<%=request.getParameter("event_id") %>, <%= session.getAttribute("memberid")%>)" ng-cloak>
					...
					<div class="mdl-layout mdl-js-layout">
						<section class="members-directory">
							<div class="page-content">															
								...
								<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center" style="color:grey;" ng-show="spin == 1"><i class="fa fa-refresh fa-spin"></i><h2> Fetching Data<span style="color:black;" id="wait">.</span></h2></div>
									<div class="panel-group d-accordion">
										<div class="panel panel-default" ng-repeat="item in getMemberForMembersDirectory | filter:search" style="height:80px;">
										<%-- <%if(session.getAttribute("memberid") != null) {%>
											<a href="#" ng-click="redirectToMemberProfile(item.memberId)">
										<%}%> --%>
											<div class="panel-heading" style="height:100%;">
												
												<img class="img-responsive avatar" src="{{item.memberProfilePicture}}" alt="" onerror="this.onerror=null; this.src='<%=request.getContextPath() %>/resources/admin/images/UserImg.png'" ng-if="item.memberProfilePicture != ''"/>
												...
												<!-- <h4 class = "panel-title texter"> {{item.firstName}} {{item.lastName}} </h4> -->
												<h4 class = "panel-title" style="color:white; margin-top: -15px;"> {{item.firstName}} {{item.lastName}} </h4>
												<h6 class = "panel-title texter" ng-show="item.businessCategoryName"  style="color:white; font-size:15px; margin-top:-5px">{{item.businessCategoryName}}</h6>
												<h6 class = "panel-title texter" ng-show="item.compName" style="color:white; font-size:12px; margin-top:-5px">{{item.compName}}</h6>
												<h6 class = "panel-title texter" ng-show="!item.businessCategoryName" style="color:white; font-size:15px; margin-top:-5px">&nbsp;</h6>
												<h6 class = "panel-title texter" ng-show="!item.compName" style="color:white; font-size:12px; margin-top:-5px">&nbsp;</h6>
