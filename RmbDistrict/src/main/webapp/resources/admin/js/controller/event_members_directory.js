app.controller('registrMembersDirectoryCtrl', function($scope, $http, $window, $filter, $location, $timeout) {
	...
	$scope.checkMemberregistration = function(id,memberid){
		...
		var link = baseUrl + 'getAllRegisteredMembersByEventID?eventid='+id;
		$http.get(link).success(function(data, status, headers, config)
		{
			$scope.getMemberForMembersDirectory = data;
			$scope.spin = 0;
			if($scope.registrationStatus !='Success' && ($scope.paymentStatus != 'Success' || $scope.reqpaid == 'Yes')){
